...
NEXT_MENU = create_menu(["Далее", "Назад"])
END_FAQ_MENU = create_menu(["Назад"])
START_MENU = create_menu(["Начать"])

# Структура переходов между меню
MENU_TRANSITIONS = {
    'main_menu': {
        'next': None,
        'text': "🎉 Главное меню",
...
# Словарь для соответствия имени меню и объекта
MENU_OBJECTS = {
    'main_menu': MAIN_MENU,
    ...
    'search_menu': None,  # Будет создаваться динамически
    'specialty_info_menu': create_menu(["Назад"]),  # Меню с одной кнопкой "Назад"
    'faq_menu': NEXT_MENU,
...
def connect_to_db():
    try:
        ...
        if connection.is_connected():
            logger.info("Успешно подключено к базе данных MySQL")
            return connection
    except Error as e:
...
def get_admission_places(grade=None, specialty_id=None):
    ...
    try:
        cursor = connection.cursor()
        if grade == 11:
            # Для 11 класса возвращаем фиксированные значения
            if specialty_id:
            ...
            # Для 9 класса или без указания класса используем данные из базы
            if specialty_id:
                query = """
...
def handle_start(message):
    welcome_text = "🎉 Добро пожаловать в бот ВятГУ! Выберите раздел, чтобы узнать всё о поступлении, оплате или общежитии! 🚀"
...
def stop(message):
    bot.send_message(message.chat.id, "👋 Работа завершена. До встречи!")
    bot.send_message(message.chat.id, "Нажмите 'Начать', чтобы вернуться", reply_markup=MENU_OBJECTS['start_menu'])
...
def handle_grade_selection(message):
    ...
    if section == "admission":
        ...
        if not all_specialties:
            bot.send_message(chat_id, "😕 Специальности для этого класса пока не найдены.", reply_markup=MENU_OBJECTS['main_menu'])
            initialize_user_state(chat_id)
...
def handle_back(message):
    ...
    # Обработка для specialty_info_menu (возврат к search_menu)
    ...
    # Обработка для search_menu (возврат к grade_menu)
    ...
    # Очищаем данные при возврате к определенным меню
...
def handle_faq_next(message):
    ...
    if not faq_list:
        bot.send_message(chat_id, "😕 Вопросы не найдены.", reply_markup=MENU_OBJECTS['main_menu'])
        initialize_user_state(chat_id)
        return
    state['faq_index'] += 1
    ...
    try:
        ...
        if state['current_menu'] == 'search_menu' and state['section'] == 'search' and text != "Назад":
            ...
            if selected_specialty:
                ...
                bot.send_message(chat_id, "🤔 Пожалуйста, выберите специальность из списка!", reply_markup=MENU_OBJECTS['search_menu'])
                return
        ...
        if text in response:
            ...
            if text.startswith('1.') and not state['grade']:
                bot.send_message(chat_id, "📚 Пожалуйста, выберите класс (9 или 11).", reply_markup=MENU_OBJECTS['grade_menu'])
                return
            ...
            bot.send_message(chat_id, "🤔 Пожалуйста, выберите раздел из меню!", reply_markup=MENU_OBJECTS['main_menu'])
    except Exception as e:
